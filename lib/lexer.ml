...
let read_char stm =
  ...
  | head :: tail ->
    { index = stm.index + 1; before = stm.next :: stm.before; next = head; after = tail }
;;
...
let unread_char stm =
  ...
  | head :: tail ->
    { index = stm.index - 1; before = tail; next = head; after = stm.next :: stm.after }
;;
...
let rec skip_blank_chars stm =
  let c = stm.next in
  if c = ' ' || c = '\t' || c = '\r' || c = '\n'
  then skip_blank_chars (read_char stm)
  else stm
;;
...
let scan_token scanner =
  ...
  let rec scan_identifier stm acc =
    ...
    if is_alpha c_next || is_digit c_next || c_next = '_'
    then scan_identifier stm_next (acc ^ Char.escaped c_next)
    else if acc = "print"
    then { stream = read_char stm; token = Some (TKeyword Print) }
  ...
  let rec scan_literal stm acc =
    ...
    let c_next = stm_next.next in
    if is_digit c_next
    then scan_literal stm_next (acc ^ Char.escaped c_next)
    else { stream = read_char stm; token = Some (TLiteral (int_of_string acc)) }
  in
  if is_alpha c
  then scan_identifier stm (Char.escaped c)
  else if is_digit c
  then scan_literal stm (Char.escaped c)
  else (
    match c with
    | '=' ->
      let stm_next = read_char stm in
      let c_next = stm_next.next in
      if c_next = '='
      then { stream = read_char stm_next; token = Some TEquals }
...
let read_token tokens =
  ...
  | Some t ->
    (match tokens.after with
     ...
     | head :: tail -> { before = t :: tokens.before; next = Some head; after = tail })
;;
...
let unread_token tokens =
  ...
  | Some t ->
    (match tokens.before with
     ...
     | head :: tail -> { before = tail; next = Some head; after = t :: tokens.after })
;;
