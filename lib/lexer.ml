...
let scan scanner =
    ...
    let rec scan_identifier stm acc =
        ...
        let _ = print_endline ("fine [" ^ Char.escaped next_c ^ "]") in
            if is_alpha next_c || is_digit next_c || next_c = '_'
            then scan_identifier stm (acc ^ (Char.escaped next_c))
            else
                if acc = "print" then {stream = read_char stm; token = Some Print}
    ...
    let rec scan_literal stm acc =
        ...
        let next_c = stm.next in
            if is_digit next_c
            then scan_literal stm (acc ^ (Char.escaped next_c))
            else
                {stream = read_char stm; token = Some (Literal (int_of_string acc))}
    in
